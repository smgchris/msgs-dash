 ...
 class ItemsTableBordered extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      fromdate: '',
      todate: '',
      stockProducts: this.props.items.filter(item=>item.quantity>0)
    };
  ...
  onClick(e) {
    ...
    if (this.state.todate !== '' && this.state.fromdate !== '' && this.state.product !== '')
      this.setState({
        stockProducts: this.props.items.filter(item =>item.quantity>0 && item.expiration_date >= this.state.fromdate && item.expiration_date <= this.state.todate + 1 && item.product.product_id === this.state.product)
      })
    else if (this.state.todate !== '' && this.state.fromdate !== '')
      this.setState({
        stockProducts: this.props.items.filter(item =>item.quantity>0 && item.expiration_date >= this.state.fromdate && item.expiration_date <= this.state.todate + 1)
      })
    else if (this.state.product !== '')
      this.setState({
        stockProducts: this.props.items.filter(item =>item.quantity>0 && item.product.product_id === this.state.product)
      })
  ...
  render() {
    ...
    const filteredProducts=this.state.stockProducts.filter(item=>item.quantity>0);
            ...
            <div className="">
              <CardBody>
                <Row>
                  ...
                  <Col lg='3'>
                    <FormGroup>
                      <Label for="fromdate">Expiring Between</Label>
