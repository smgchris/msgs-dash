...
class StockGraph extends React.Component {
    ...
    render() {
        const { stock, currentStock,fromdate, todate } = this.props;

        const stockFilter=stock.filter(item => item.created_date.split(' ')[0]>=fromdate-1&&item.created_date.split(' ')[0]<=todate+1)
        ...
        if(fromdate!==''&&todate!==''&& typeof stock[0] != 'undefined') {

           
            for (var i = 0; i <= (new Date(todate).getTime()-new Date(fromdate).getTime())/86400000; i++) {
                //var cdate=new Date(new Date().setDate(new Date(todate).getDate()-i)).toISOString().slice(0, 10)
                var cdate=new Date(new Date(todate).getTime()-(i*86400000)).toISOString().slice(0, 10)
        ...
        }else if(typeof stock[0] != 'undefined'){
            for (var i = 0; i <= 7; i++) {
            ...
            <div className="widget-chart p-0">
                <div className="widget-chart-content">
                    <div className="widget-description mt-0 text-warning">
                        ...
                        <span className="text-muted opacity-8 pl-1">Stock purchases  {fromdate!==''&&todate!==''?'from '+fromdate+' to '+todate:'in the past 7 days'}</span>
                    </div>
                </div>
